...
# Supports: Gemini, OpenRouter, Groq, Together AI, Cerebras AI, Novita AI, Ollama Cloud, Cloudflare Workers AI
...
# Get Account ID from URL: dash.cloudflare.com/ACCOUNT_ID/
# Get API Token from: Profile → API Tokens → Create Token (Workers AI permission)
...
function print_usage() {
  ...
  echo -e ""
  echo -e "${COLOR_INFO}Finding Model Identifiers:${COLOR_RESET}"
  ...
  echo -e " ${COLOR_BOLD}${COLOR_AI}$0 openrouter llama${COLOR_RESET}"
  echo -e ""
  echo -e "${COLOR_IMAGE}Image Support:${COLOR_RESET}"
  echo -e " Supports JPEG, PNG, GIF, WebP. Max ${MAX_IMAGE_SIZE_MB}MB per image."
...
case "$PROVIDER" in
    ...
    cloudflare)
        if [[ -z "$CLOUDFLARE_ACCOUNT_ID" || -z "$CLOUDFLARE_API_TOKEN" ]]; then
            echo -e "${COLOR_WARN}!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!${COLOR_RESET}" >&2
            echo -e "${COLOR_WARN}!! WARNING: Cloudflare requires both ACCOUNT_ID and API_TOKEN.${COLOR_RESET}" >&2
            echo -e "${COLOR_WARN}!! Please edit the script and set CLOUDFLARE_ACCOUNT_ID and${COLOR_RESET}" >&2
            echo -e "${COLOR_WARN}!! CLOUDFLARE_API_TOKEN variables.${COLOR_RESET}" >&2
            echo -e "${COLOR_WARN}!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!${COLOR_RESET}" >&2
            key_check_status=1
        else
...
case "$PROVIDER" in
    ...
    cloudflare)
        ...
        MODELS_AUTH_HEADER="Authorization: Bearer ${CLOUDFLARE_API_TOKEN}"
        MODELS_EXTRA_HEADERS+=("-H" "Content-Type: application/json")
        MODELS_POST_DATA='{"task":{"name":"Text Generation"}}'
        JQ_QUERY='.result[] | select(.task.name == "Text Generation") |.name'
        ;;
esac

if [[ -n "$MODELS_POST_DATA" ]]; then
    model_curl_args=(-sS -L -X POST "$MODELS_URL" -d "$MODELS_POST_DATA")
else
...
case "$PROVIDER" in
    gemini)
        ...
        IS_OPENAI_COMPATIBLE=false
        ;;
    openrouter|groq|together|cerebras|novita|ollama|cloudflare)
        ...
        case "$PROVIDER" in
            openrouter)
                ...
                ;;
            groq)
                CHAT_API_URL="$GROQ_CHAT_URL"
                CHAT_AUTH_HEADER="Authorization: Bearer ${API_KEY}"
                ;;
            together)
                CHAT_API_URL="$TOGETHER_CHAT_URL"
                CHAT_AUTH_HEADER="Authorization: Bearer ${API_KEY}"
                ;;
            cerebras)
                CHAT_API_URL="$CEREBRAS_CHAT_URL"
                CHAT_AUTH_HEADER="Authorization: Bearer ${API_KEY}"
                ;;
            novita)
                CHAT_API_URL="$NOVITA_CHAT_URL"
                CHAT_AUTH_HEADER="Authorization: Bearer ${API_KEY}"
                ;;
            ollama)
                ...
                ;;
            cloudflare)
                CHAT_API_URL="https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/ai/v1/chat/completions"
                CHAT_AUTH_HEADER="Authorization: Bearer ${CLOUDFLARE_API_TOKEN}"
                ;;
        esac
        ;;
esac
...
if [[ "$PROVIDER" == "cloudflare" ]]; then
    echo -e "${COLOR_INFO}Account ID:${COLOR_RESET} ${CLOUDFLARE_ACCOUNT_ID}"
fi
...
while true; do
    ...
    allowed_conversational_messages=$(( MAX_HISTORY_MESSAGES ))
